namespace LLVMSharp
{
    ...
    internal class StringMarshaler : ICustomMarshaler
    {
        ...
        public static IntPtr[] MarshalArray(string[] inputs)
        {
            IntPtr[] marshaledStrings = new IntPtr[inputs.Length];

            for (int i = 0; i < marshaledStrings.Length; i++)
            {
                marshaledStrings[i] = staticInstance.MarshalManagedToNative(inputs[i]);
            }

            return marshaledStrings;
        }
